...
function trends(input = {}) {
  ...
  const operationsTrend = data.projects.length ? "tracked" : "Add job history";
  const customerTrend = data.customers.length ? "tracked" : "Add customer history";
  const marketingTrend = data.leads.some((lead) => /website|google|seo|instagram|facebook|tiktok|social/i.test(String(lead.source || ""))) ? "active" : "Add lead source tracking";
...
function trendFromDates(items, field) {
  if (!items.length) return "Add more data";
...
function trendFromValues(items) {
  ...
  if (values.length < 2) return "Add one more value";
  return values[values.length - 1] >= values[0] ? "up" : "down";
}

function trendFromStatuses(estimates) {
  if (!estimates.length) return "Add estimates";
